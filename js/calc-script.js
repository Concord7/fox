document.addEventListener('DOMContentLoaded', () => {
	...
	// 1. Обработка ввода в поле даты: ставим точки сами и не даем писать буквы
    birthdateInput.addEventListener('input', function(e) {
        // Удаляем всё, кроме цифр
        let value = this.value.replace(/\D/g, '');
        
        // Обрезаем, если больше 8 цифр (ддммгггг)
        if (value.length > 8) value = value.slice(0, 8);
        
        // Форматируем: добавляем точки
        if (value.length >= 5) {
    ...
    // 2. Обработка нажатия Backspace в поле даты (чтобы удобно стирать точки)
    birthdateInput.addEventListener('keydown', function(e) {
        if (e.key === 'Backspace' && (this.value.endsWith('.') )) {
             // Если стираем точку, удаляем и цифру перед ней для удобства? 
             // Нет, стандартное поведение обычно устраивает, оставим пустым пока.
        }
    });
    ...
    // Переменная для сохранения текста результатов (чтобы передать в форму)
    let resultsTextForForm = "Результаты чакр: ";

    // 1. Выводим сами чакры (твой старый код)
    for (const chakraName in chakraPercentages) {
        ...
        // Собираем текст для скрытого поля (бонус)
        resultsTextForForm += `${chakraName}: ${percentageRange}; `;
    }

    // 2. Создаем кнопку "Узнать что это значит"
    const actionButton = document.createElement('button');
    actionButton.innerText = 'Узнать что это значит';
    
    // Добавляем класс стиля такой же, как у кнопки "Рассчитать"
    actionButton.classList.add('btn-calc'); 
    
    // Говорим кнопке занять все доступные колонки (от 1 до последней)
    actionButton.style.gridColumn = "1 / -1"; 
    
    // Выравниваем кнопку по центру этого пространства
    actionButton.style.justifySelf = "center"; 
    
    // Отступ сверху
    actionButton.style.marginTop = '20px'; 
    
    // Если хочешь кнопку во всю ширину - раскомментируй строку ниже, 
    // а justifySelf убери. Но для "по центру" оставь как есть.
    // actionButton.style.width = '100%';

    // 3. МАГИЯ BOOTSTRAP: Атрибуты для переключения окон
    // Эта команда говорит: "Открой окно #exampleModal"
    actionButton.setAttribute('data-bs-toggle', 'modal');
    actionButton.setAttribute('data-bs-target', '#exampleModal');

    // 4. Добавляем обработчик клика для передачи данных в форму (Бонус)
    actionButton.addEventListener('click', function() {
        // Находим скрытое поле в форме контактов
		...
		if (birthDayField) {
			birthDayField.value = birthdateInput.value;
		}
    });

    // 5. Добавляем кнопку в конец контейнера с результатами
    resultsContainer.appendChild(actionButton);
	}
    ...
    calculateBtn.addEventListener('click', () => {
        ...
        // Проверяем, заполнена ли дата полностью (10 символов: 01.01.2000)
        if (dateStr.length !== 10) {
            alert("Пожалуйста, введите полную дату рождения (ДД.ММ.ГГГГ)");
            return;
        }
        ...
        // 2. Проверяем месяц (не больше 12)
        if (month < 1 || month > 12) {
            alert("Месяц должен быть от 01 до 12");
            return;
        }

        // 3. Проверяем день (не больше 31)
        if (day < 1 || day > 31) {
            alert("День должен быть от 01 до 31");
            return;
        }

        // 4. Проверяем год (не будущее и не древность)
        const currentYear = new Date().getFullYear();
        if (year < 1900 || year > currentYear) {
            alert("Год рождения должен быть реальным (от 1900 до текущего)");
            return;
        }

        // 5. САМАЯ ХИТРАЯ ПРОВЕРКА: Существует ли такая дата?
        // (Ловит 30 февраля, 31 апреля и т.д.)
        const birthDate = new Date(year, month - 1, day);
        
        // Если мы создали дату "31.02.2000", JS сам превратит её в "02.03.2000".
        // Мы проверяем: совпадает ли то, что получилось, с тем, что ввели.
        if (birthDate.getFullYear() !== year || 
            birthDate.getMonth() + 1 !== month || 
            birthDate.getDate() !== day) {
            alert("Такой даты не существует в календаре! Проверьте число.");
            return;
        }

        const chakraPercentages = calculateChakras(birthDate);
